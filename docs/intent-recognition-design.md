...
2. Scores below 0.8 invoke a one-turn Claude classifier. Claude results with confidence below 0.5 fall back to the rule result.
...
Intent recognition runs before prompt construction. Durable history is reduced to the context budget, terminal facts are collected through the bound remote PTY, and history/task/milestone/tool-result sections are added to the user message before the Claude Agent loop begins.
