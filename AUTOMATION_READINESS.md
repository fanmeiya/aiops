...
The service can perform automated operations when all three external systems are configured: MySQL, the DeepSeek API, and a reachable SSH server. This audit does not claim live end-to-end certification without those systems.

## Verified in code

- Local command execution is disabled; the only enabled execution tool is the in-process `executeCommand` tool.
